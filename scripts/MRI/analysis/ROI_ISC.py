...
sub_atlas = {
    'amygdala': [9, 19], # Left and right amygdala
    'hippocampus': [8, 18] # Left and right hippocampus
}
...
movie_segments = { # Valence segments via hidden markov model
    'A': [86, 96, 111, 157, 242, 394, 421, 602, 683, 714, 796, 979, 1092, 1197, 1207, 1237, 1279, 1383, 1465, 1527, 1604],
    'B': [9, 68, 81, 109, 146, 175, 405, 657, 968, 1137, 1357, 1455, 1539, 1617, 1661, 1704],
    'C': [9, 43, 62, 301, 336, 372, 419, 600, 888, 955, 998, 1291, 1356, 1541, 1647, 1653]
}

"""
movie_segments = { # Volatility segments via hidden markov model
    'A': [],
    'B': [],
    'C': []
}
...
low_conflict = set([str(sub) for sub in low_conflict])
    ...
    for roi in roi_list: # Iterate through ROIs
        ...
        for subject_file in subject_files:
            ...
            for segment_ind, segment_start in enumerate(movie_segments[movie][:-1]):
                ...
                # Calculate segment subject difference
                sub_seg_difference = np.nanmean(roi_avg[segment_start:segment_end] - subject_roi[segment_start:segment_end])
                ...
                csv_contents.append([subject, movie, roi, segment_start, segment_end, conflict, valence_mean, volatility_mean, sub_seg_difference])

print("Saving file...")
header = ['Subject', 'Movie', 'ROI', 'Segment Start', 'Segment End', 'Conflict Group', 'Segment Average Valence', 'Segment Average Volatility', 'Average Subject Difference']
with open(f"{deviation_dir}subject-ROI_ISC_liver.csv", 'w') as f:
    writer = csv.writer(f)
