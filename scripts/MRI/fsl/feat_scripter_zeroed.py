...
for sub_line in tqdm(fsl_subs):
    ...
    with open(
        str(os.path.dirname(os.path.realpath(sys.argv[0])))+"/fsl_template_changeme_zeroed.fsf", 'r')\
            as template:
        with open(
            "/scratch/claytons/MRI_data_clean/derivatives/fmriprep/{}/feat_script_zeroed.fsf".format(
    ...
    print("Wrote:", "{}/feat_script_zeroed.fsf".format(os.path.split(sub_line)[0]))
