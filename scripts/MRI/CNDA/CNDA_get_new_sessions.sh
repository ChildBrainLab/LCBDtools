...
if [ -f "/data/perlman/moochie/study_data/CARE/CNDA_sessions_new.txt"]; then
	...
	mv /data/perlman/moochie/study_data/CARE/CNDA_sessions_new.txt \
		/data/perlman/moochie/study_data/CARE/CNDA_sessions.txt
fi

echo "Looking for new sessions:"
echo "========================="

bash /data/perlman/moochie/github/LCBDtools/scripts/MRI/CNDA/CNDA_get_session.sh \
	/data/perlman/moochie/study_data/CARE/CNDA_sessions_new.txt

# for any new sessions (diff between old and new)
# run python emailer to remind RAs to go enter usability
for new_session in `comm -23 <(sort "/data/perlman/moochie/study_data/CARE/CNDA_sessions_new.txt") <(sort "/data/perlman/moochie/study_data/CARE/CNDA_sessions.txt")`; do
