...
/opt/conda/envs/fmriprep/bin/fmriprep /input /tmp/$JOBFOLDER/output participant --participant-label sub-$SUBJECT --session-id ses-$SESSION -w /tmp/$JOBFOLDER/scratch --fs-license-file /freesurfer/license.txt --output-spaces MNI152NLin6Asym:res-2 MNI152NLin2009cAsym:res-2 MNIPediatricAsym:cohort-2:res-2 --n_cpus 16 --mem 24 --low-mem --skip-bids-validation 
