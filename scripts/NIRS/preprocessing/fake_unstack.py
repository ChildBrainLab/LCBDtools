...
filenames = ['wct_full_permuted_values.csv', 'wct_full_permuted_values_new.csv', 'wct_full_permuted_values_new_1.csv']
...
def unstack_file(filename, folder, study, unstack = True, DOI = True, ROI = True):
	...
	for datum in data:
		...
		if datum[1] == datum[2]:
			...
			parent = str(random_start) + datum[1][2::-1] + str(random_end)
			child = str(random_start) + datum[2][2::-1] + str(random_end)
		else:
			random_start = random.randint(0, 9)
			random_end = random.randint(0, 9)
			parent = str(random_start) + datum[1][2::-1] + str(random_end)
		...
		if dyad == DOI: # Check if dyad status matches what we want to save
			...
			if ROI == False:
				stacked_data[f"{parent}-{child}"][block] = [random.uniform(0.3, 0.5) for thing in datum[channel_start:]] # Save block data
			else:
				...
				for region, channels_ind in ROIs.items():
					...
					stacked_data[f"{parent}-{child}"][block] += [random.uniform(0.3, 0.5)]
...
for study in studys:
	folder = f'/data/perlman/moochie/analysis/{study}/Fake_Analysis/'
